...
const Table = ({newUsers}) => {
  ...
  if (typeof window !== 'undefined') {
    intialValues = JSON.parse(localStorage.getItem('filteredCols')) ?
    JSON.parse(localStorage.getItem('filteredCols')) : [] ;
}
  

  const [filteredColmns ,setFilteredColmns ] = useState(intialValues)

  if(filteredColmns?.[0]){
    let elements ;
    for(let i=0 ; i < filteredColmns.length ; i++ ){
        let element = document.querySelectorAll(`#${filteredColmns[i]}`);
  ...
  const addRemoveCol = (type)=> {
    ...
    if (checkBox.checked == true){
      let newFiltered = [...filteredColmns];
      newFiltered.push(type)
      localStorage.setItem('filteredCols',JSON.stringify(newFiltered))
      setFilteredColmns(newFiltered)
      ...
      let newFiltered = filteredColmns.filter(item => item !== type)
      localStorage.setItem('filteredCols',JSON.stringify(newFiltered))
      setFilteredColmns(newFiltered)
